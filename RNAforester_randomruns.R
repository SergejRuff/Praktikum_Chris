...
rna_forester_alignment <-function(fasta_directory){

  # List all .fasta files in the directory
  fasta_files <- list.files(path = fasta_directory, pattern = "\\.fasta$", full.names = TRUE)

  # Initialize an empty list to store sequences and their dot-bracket notations
  sequences_and_dotbrackets <- list()


  n <- length(results_list)/2
  ...
  # Initialize vectors to store row and column names
  row_names <- character(n)  # Create an empty character vector of length n
  col_names <- character(n)  # Create an empty character vector of length n

  # Initialize row and column indices
  i <- 1
  j <- 1

  for (z in 1:length(fasta_files)){

    # Read the contents of the .fasta file
    fasta_content <- readLines(fasta_files[[z]])

    name_subject <- fasta_content[[12]]
    name_pattern <- fasta_content[[15]]
    sequence_subject <- fasta_content[[13]]
    sequence_pattern <- fasta_content[[16]]
    sequence_score <- as.numeric(gsub("global optimal score:","",fasta_content[[11]] ))
    subject_global <- fasta_content[[14]]
    pattern_global <- fasta_content[[17]]

    # Create a data frame
    df_forester <- data.frame(
      ...
      Pattern_Global = pattern_global,
      stringsAsFactors = FALSE
    )

    # Store the sequence and its name in a list
    sequences_and_dotbrackets[[paste( name_subject,"_",name_pattern)]] <- t(df_forester)
    ...
    row_names[i] <- name_subject
    col_names[j] <- name_pattern
  ...
  rownames(lev_distance_matrix_UTR_global) <- row_names
  colnames(lev_distance_matrix_UTR_global) <- col_names
  rownames(lev_distance_percentage_matrix_UTR_global) <- row_names
  colnames(lev_distance_percentage_matrix_UTR_global) <- col_names

  return(list(lev_distance_percentage_matrix_UTR_global=lev_distance_percentage_matrix_UTR_global,
              lev_distance_matrix_UTR_global=lev_distance_matrix_UTR_global,
              dataframes= sequences_and_dotbrackets))
}
...
folder_name <- paste0("randomRNAforester_files_",current_date)
...
for (k in 1:range){
  ...
  for (i in 1:length(results_list)) {
    ...
    dotfasta_folder <- "rnafoldfasta_ergebnisse"
    dotfasta_folder <- file.path(folder_path_, dotfasta_folder)

    if (!dir.exists(dotfasta_folder)) {
      dir.create(dotfasta_folder, recursive = TRUE)
    }

    setwd(dotfasta_folder)

    # Extract the headers and sequences
    headers <- names(results_list)[i]
    headers <- gsub("X", ">", headers)

    if (grepl(">5_UTR", headers)) {

      # Create a CT-results folder for 5UTR if not already created
      dotfasta_5utr <- "rnafoldfasta_5utr"
      dotfasta_5utr <- file.path(dotfasta_folder, dotfasta_5utr)

      if (!dir.exists(dotfasta_5utr)) {
        dir.create(dotfasta_5utr, recursive = TRUE)
      }

      # Set the working directory to 5UTR folder
      setwd(dotfasta_5utr)

    } else {

      # Create a CT-results folder for 3UTR if not already created
      dotfasta_3utr <- "rnafoldfasta__3utr"
      dotfasta_3utr <- file.path(dotfasta_folder, dotfasta_3utr)

      if (!dir.exists(dotfasta_3utr)) {
        dir.create(dotfasta_3utr, recursive = TRUE)
      }

      # Set the working directory to 3UTR folder
      setwd(dotfasta_3utr)
    }

    sequences_fdb <- results_list[[i]][1:2, ]
    header_name <- gsub(">", "", headers)
    output_file <- paste0(header_name, ".fasta")

    # Open the output file for writing
    file_conn <- file(output_file, "w")
    writeLines(headers, file_conn, sep = "\n")

    # Loop through each sequence and write the headers and sequences to the file
    for (n in 1:2) {
      sequence_fdb <- as.character(sequences_fdb[n])

      # Write sequence to the file without leading space
      writeLines(sequence_fdb, file_conn, sep = "\n")
    }
  ...
  rna_forester_input <- c(dotfasta_5utr,dotfasta_3utr)


  for(i in 1:length(rna_forester_input)){

    # Create a CT-results folder if not already created
    forester_folder <- "forester_results"
    forester_folder <- file.path(rna_forester_input[i], forester_folder)

    if (!dir.exists(forester_folder)) {
      dir.create(forester_folder, recursive = TRUE)
    }

    setwd(forester_folder)

    # Replace only the uppercase drive letter 'A' with 'a'
    fdb_path <- gsub("^A:", "a", rna_forester_input[i] )

    # Replace the Windows drive letter with the WSL path
    fdb_path  <- paste0("/mnt/",fdb_path,"/" )


    # Define the Bash script path
    bash_script_path_ <- "/mnt/a/Praktikum_Chris/RNAforester_align.sh"

    # Build the command with arguments
    command <- sprintf("bash -x \"%s\" \"%s\"", shQuote(bash_script_path_), shQuote(fdb_path ))
  ...
  ###########################################
  ## extract dot sequences from RNAforester #
  #         and perform alignment           #
  ###########################################



  rna_forester_result_5 <-paste0(dotfasta_5utr,"/forester_results/results_folder")
  rna_forester_result_3 <-paste0(dotfasta_3utr,"/forester_results/results_folder")

  ### call function
  rnaforester_5_results <- rna_forester_alignment(rna_forester_result_5)
  rnaforester_3_results <- rna_forester_alignment(rna_forester_result_3)
  ...
  write.csv(rnaforester_3_results[["lev_distance_matrix_UTR_global"]], file = paste0(folder_path_,"/allignmentscores/","threeUTR_global_lev_distance_matrix.csv"))

  # Write the matrix to a CSV file
  write.csv(rnaforester_3_results[["lev_distance_percentage_matrix_UTR_global"]], file = paste0(folder_path_,"/allignmentscores/","threeUTR_global_percentage_lev_distance_matrix.csv"))


  # Write the matrix to a CSV file
  write.csv(rnaforester_5_results[["lev_distance_matrix_UTR_global"]], file = paste0(folder_path_,"/allignmentscores/","fiveUTR_global_lev_distance_matrix.csv"))

  # Write the matrix to a CSV file
  write.csv(rnaforester_5_results[["lev_distance_percentage_matrix_UTR_global"]], file = paste0(folder_path_,"/allignmentscores/","fiveUTR_global_percentage_lev_distance_matrix.csv"))
  ...
  tree_3_global <- utr_nj(rnaforester_3_results[["lev_distance_percentage_matrix_UTR_global"]],filename="threeUTR_njtree_global.png",test="Global Alignment of 3´-UTR")

  tree_5_global <- utr_nj(rnaforester_5_results[["lev_distance_percentage_matrix_UTR_global"]],filename="fiveUTR_njtree_global.png",test="Global Alignment of 5´-UTR")
